...
class Visualizer():
    def __init__(self, opt):
        ...
        print(f"images are stored in {self.output_dir}")
        ...
        if self.use_html: # save images to a html file
            for label, image_numpy in visuals.items():
                if self.isTrain:
                    ...
                    img_path = os.path.join(self.output_dir, 'epoch%.3d_%s.png' % (epoch, label))
                else:
                    # img_path = os.path.join(self.img_dir, '%d.png' % (epoch))
                    img_path = os.path.join(self.output_dir, '%d.png' % (epoch))
